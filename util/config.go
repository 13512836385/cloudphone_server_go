...
func GetVolcConfig() (string, string, error) {
	...
	var config struct_package.VolcConfig
...
func GetAcepConfig() *struct_package.AcepConfig {
	homeDir, err := os.Getwd()
	if err != nil {
		panic(err)
	}
	...
	var config struct_package.AcepConfig
	err = json.Unmarshal(data, &config)
	if err != nil {
		panic(err)
	}
